...
public class CreateChallenge extends base {
	...
	public void addnewChallenge() throws IOException, InterruptedException {
		...
		build.moveToElement(add.insertChallengeName()).click()
				.sendKeys(" Automated script for challenge name , this is only for automation testing ").build()
				.perform();
		build.moveToElement(add.insertChallengeDESC()).click().sendKeys(
				"Automated script for challenge name , this is only for automation testing  name challenge name Challenge name Challenge name ")
				.build().perform();
		...
		wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//small[text()=' Idea Submission in progress ']")));
		// driver.findElement(By.xpath("//small[text()=' Idea Submission in progress
		// ']")).isDisplayed();
